...
// The EnsembleProposerFunc type is an adapter to allow the use of ordinary function as a Querier.
type EnsembleProposerFunc func(context.Context, *ent.EnsembleProposerQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f EnsembleProposerFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.EnsembleProposerQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.EnsembleProposerQuery", q)
}

// The TraverseEnsembleProposer type is an adapter to allow the use of ordinary function as Traverser.
type TraverseEnsembleProposer func(context.Context, *ent.EnsembleProposerQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseEnsembleProposer) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseEnsembleProposer) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.EnsembleProposerQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.EnsembleProposerQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.EnsembleProposerQuery:
		return &query[*ent.EnsembleProposerQuery, predicate.EnsembleProposer, ensembleproposer.OrderOption]{typ: ent.TypeEnsembleProposer, tq: q}, nil
