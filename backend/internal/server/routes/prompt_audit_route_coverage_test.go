...
func TestEveryGatewayPOSTRouteIsClassifiedForPromptAuditCoverage(t *testing.T) {
	...
	audited := map[string][]string{
		...
		"/chat/completions":         {"gateway_handler_chat_completions.go", "openai_chat_completions.go", "ensemble_chat_completions.go"},
		"/ensemble/test":            {"ensemble_chat_completions.go"},
