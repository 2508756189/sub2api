...
describe('admin UsageTable tooltip', () => {
  ...
  it.each(['token', 'image', 'per_request'])('keeps eight decimal places in %s cost details', async (billingMode) => {
    ...
    expect(amounts).toEqual(expect.arrayContaining([
      `${billingSymbol}0.00000001`, `${billingSymbol}0.00000002`, `${billingSymbol}0.00000003`, `${billingSymbol}0.00000004`,
      `${billingSymbol}0.00000005`, `${billingSymbol}0.00000006`, `${billingSymbol}0.00000022`, `${billingSymbol}0.00000042`, `${billingSymbol}0.00000018`,
    ]))
    if (billingMode === 'image') expect(amounts).toContain(`${billingSymbol}0.00000011`)
    wrapper.unmount()
  })
  ...
  it('uses eight decimal places for missing cost values', async () => {
    ...
    const triggers = wrapper.findAll('.group.relative')
    await triggers[triggers.length - 1].trigger('mouseenter')
    const amounts = wrapper.get('.fixed').findAll('span').map(span => span.text()).filter(text => text.startsWith(billingSymbol))
    expect(amounts).toEqual([`${billingSymbol}0.00000000`, `${billingSymbol}0.00000000`, `${billingSymbol}0.00000000`, `${billingSymbol}0.00000000`])
    wrapper.unmount()
  })
