<template>
  <div class="space-y-6">
    <section class="overflow-hidden rounded-2xl border border-emerald-950/10 bg-gradient-to-br from-white via-[#f7fbf9] to-[#eef7f3] dark:border-dark-700 dark:from-dark-900 dark:via-dark-900 dark:to-dark-950">
      <div class="grid gap-6 p-5 lg:grid-cols-[1fr_auto] lg:items-end lg:p-7">
        <div>
          <div class="mb-3 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white/80 px-3 py-1 text-xs font-semibold text-emerald-700 dark:border-emerald-800/50 dark:bg-dark-800 dark:text-emerald-300">
            <img src="/ctyun-logo.svg" alt="Tianyi Cloud" class="h-4 w-4 rounded" />
            TOKENPORT · SKILL MARKET
          </div>
          <h1 class="text-[26px] leading-tight font-extrabold tracking-tight text-gray-950 dark:text-white sm:text-3xl">可复用的智能体能力资产</h1>
        ...
        <div class="grid grid-cols-3 gap-3 text-center">
          <div class="rounded-xl border border-emerald-950/8 bg-white px-4 py-3 shadow-sm dark:border-dark-600 dark:bg-dark-800">
    ...
    <section class="rounded-2xl border border-emerald-950/10 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-800/70 lg:p-5">
      ...
      <div class="mt-4 flex flex-wrap gap-2">
        <button
          ...
          :class="category.value === activeCategory
            ? 'border-emerald-500 bg-emerald-50 text-emerald-700 shadow-sm dark:border-emerald-500/60 dark:bg-emerald-900/25 dark:text-emerald-300'
            : 'border-gray-200 text-gray-600 hover:border-emerald-300 hover:bg-emerald-50/50 dark:border-dark-600 dark:text-gray-300 dark:hover:border-emerald-700 dark:hover:bg-emerald-900/10'"
    ...
    <div v-if="loading" class="rounded-2xl border border-gray-200 bg-white py-16 text-center text-sm text-gray-500 dark:border-dark-700 dark:bg-dark-800/70 dark:text-gray-400">正在加载技能市场...</div>
    ...
    <div v-else-if="!filteredSkills.length" class="rounded-2xl border border-gray-200 bg-white py-16 text-center text-sm text-gray-500 dark:border-dark-700 dark:bg-dark-800/70 dark:text-gray-400">没有符合条件的技能。</div>
    <div v-else class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      <article
        v-for="skill in filteredSkills"
        :key="skill.id"
        class="group flex min-h-[280px] flex-col rounded-2xl border border-emerald-950/10 bg-white p-5 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-md dark:border-dark-700 dark:bg-dark-800/70 dark:hover:border-emerald-600"
      >
